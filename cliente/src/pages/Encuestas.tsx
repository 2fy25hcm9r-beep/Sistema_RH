...
export default function Encuestas() {
  ...
  useEffect(() => {
    if (modalAcceso !== null) {
      ...
      api
        .get(`/encuestas/${modalAcceso}/empleados-no-asignados/`)
  ...
  const activarEncuesta = async (id: number) => {
    await api.post(`/encuestas/${id}/activar/`);
    fetchEncuestas();
  };

  const cerrarEncuesta = async (id: number) => {
    await api.post(`/encuestas/${id}/cerrar/`);
    fetchEncuestas();
  };
  ...
  const asignarEmpleados = async () => {
    ...
    try {
      await api.post(
        `/encuestas/${modalAcceso}/asignar-empleados/`,
        {
          numeros_empleado: lista,
        }
      );

      toast.success(`${lista.length} empleado(s) asignado(s)`);
          ...
          {encuestas.map((enc) => (
            <div key={enc.id} className={styles.card}>
              ...
              <div className={styles.acciones}>
                ...
                {/* VER ENCUESTA */}
                <a
                  href={`${FRONTEND_URL}/encuestas/${enc.id}`}
                ...
                {!esVisor && (
                  <a
                    href={`${FRONTEND_URL}/encuesta${
                      obtenerOrdenEncuesta(enc.titulo) <= 3
                        ? obtenerOrdenEncuesta(enc.titulo)
                        : enc.id
                    }/responder`}
                    className={styles.btnAccion}
                    target="_blank"
                ...
                {/* ACTIVAR / CERRAR / ABRIR */}
                {!esVisor && (
                  <>
                    {enc.estado === 'borrador' && (
                      <button
                        onClick={() => activarEncuesta(enc.id)}
        ...
        >
          ...
          >
            ...
            <p className={styles.hint}>
              Selecciona los empleados que podrán responder esta encuesta.
            </p>
            ...
            {loadingEmpleados ? (
              ...
              <select
                multiple
                size={8}
                className={styles.selectEmpleados}
                value={Array.from(empleadosSeleccionados)}
                onChange={(e) => {
            ...
            <div className={styles.seleccionInfo}>
              {empleadosSeleccionados.size}{' '}
              empleado(s) seleccionado(s)
            </div>
